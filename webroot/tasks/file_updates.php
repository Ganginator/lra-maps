<?php
	...
	if(!defined('APP')) { define('APP', dirname(dirname(dirname(__FILE__))).DS); }
			...
			if(isset($file_info->updated)) {
				...
				if($file_time>$check_file_time) {
					...
					while(($plot = fgetcsv($fh, 1000, ",")) !== false) {
						// echo 'Original: '.ucwords(strtolower($plot[0])).'<br />';
						if($plot[0]!=='Address' && count($plot)==8) {
							...
							if($plot_check!==NULL) {
								...
								$plot_check->updated = $update_datetime;
								$plot_check->save();
							} else {
					...
					$file_info->query('UPDATE `files` SET `updated`="'.$update_datetime.'" WHERE `filename`="'.$file.'"');
					$file_info->query('DELETE from `plots` WHERE type="'.$file_info->plot_type.'" AND updated<>"'.$update_datetime.'"');
				}
			}
		}
	}
